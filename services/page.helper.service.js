     ...
     function pagesHelper($http, logger, $parse, $compile, $state, cardData, $rootScope, $timeout, Pages, Authentication) {
          ...
          function updatePageBuilder(scope) {
               ...
               $timeout(function () {
                    ...
                    page.$update({pageId: scope.page._id}/*, scope.newPage*/)
                        .then(function (page) {
                                   ...
                                   page.$update({pageId: scope.newPage._id}, function (page) {
                                        ...
                                        if (status == "Live") {
                                             if (scope.newPage.client.production_refresh_url != undefined) {
                                                  $http.post(scope.newPage.client.production_refresh_url).then(function(res) { logger.success('Production CMS refreshed');});
                                             } else {
                                                 logger.success("Production CMS URL doeesn't exist."); 
                                             }
                                        } else if (status == "Preview") {
                                             if (scope.newPage.client.beta_refresh_url != undefined) {
                                                  $http.post(scope.newPage.client.beta_refresh_url).then(function(res) { logger.success('Beta CMS refreshed');});
                                             } else {
                                                 logger.success("Beta CMS URL doeesn't exist."); 
                                             }
                                        }
                                        $state.reload();
                                   })
                         ...
                         if ( count == scope.clonedPage.cards.length) {
                              ...
                              $timeout(function () {
                                   ...
                                   page.$update({pageId: scope.page._id}, function (page) {
                                        logger.success("Page exported successfully");
                                        var pro_refreshUrl = scope.newPage.client.production_refresh_url || '';
                                        var beta_refreshUrl = scope.newPage.client.beta_refresh_url || '';
                                        console.log(beta_refreshUrl);
                                        console.log(page);
                                        if (page.status == "Live" ) {
                                             if (pro_refreshUrl != '') {
                                                  $http.post(pro_refreshUrl).then(function(res) { console.log(res); logger.success('Production CMS refreshed');});
                                             } else {
                                                 logger.success("Production CMS URL doeesn't exist."); 
                                             }
                                        } else if (page.status == "Preview" ) {
                                             if (beta_refreshUrl != '') {
                                                  $http.post(beta_refreshUrl).then(function(res) { console.log(res); logger.success('Beta CMS refreshed');});
                                             } else {
                                                 logger.success("Beta CMS URL doeesn't exist."); 
                                             }
                                        }  
